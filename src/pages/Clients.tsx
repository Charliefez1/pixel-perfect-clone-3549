...
export default function Clients() {
  return (
    <>
      <PageHeader title="Clients" searchPlaceholder="Search clients..." actionLabel="New Client" />
              ...
              <TableBody>
                {clients.map((c) => (
                  <TableRow key={c.id} className="cursor-pointer">
                    ...
                    <TableCell className="text-sm text-muted-foreground">{c.contacts}</TableCell>
                    <TableCell><Badge variant="secondary">{c.sector}</Badge></TableCell>
                    <TableCell className="text-sm">{c.deals}</TableCell>
                    <TableCell className="text-sm font-medium">£{c.totalValue.toLocaleString()}</TableCell>
                    <TableCell>
